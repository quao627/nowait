...
reviews <- read_csv("/Users/aoqu/Desktop/NoWait_Research/Robustness/reviews(digital_score).csv")
```

```{r}
file <- "match(digital_score).csv"
caliper <- 0.3
ratio <- 2
time <- ymd("2017-2-28")
...
#slack_cols <- c("2015_sale", "2016_sale", "2017_sale", "2018_sale", #"2019_sale")
#data <- data %>% drop_na(any_of(slack_cols))
period <- 730
...
if (nrow(df_regression_1)>0){
  model1 <- glm(complain_line ~ After + Treated + Treated_X_After + nfriends + useful + elite + nphotos_review + checkin + Fog_index + Avg_word_length + Subjectivity + Sent_length + Food_quality + Atmosphere + digital_score, data = df_regression_1, family = "binomial")
...
if (nrow(df_regression_1)>0){
  model4 <- glm(complain_service ~ After + Treated + Treated_X_After + nfriends + useful + elite + nphotos_review + checkin + Fog_index + Avg_word_length + Subjectivity + Sent_length + Food_quality + Atmosphere + digital_score, data = df_regression_1, family = "binomial")
...
model7 <- glm(ratings ~ After + Treated + Treated_X_After + nfriends + useful + elite + nphotos_review + checkin + Fog_index + Avg_word_length + Subjectivity + Sent_length + Food_quality + Atmosphere + digital_score, data = df_regression_1, family = "gaussian")

```


```{r}
stargazer(model7, title="Regression Results(Digital Score)",
column.labels=c("Service"),
keep.stat=c("aic", "n"), ci=FALSE, star.cutoffs = c(.05, .01, .001), single.row=TRUE)
```
