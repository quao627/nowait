...
reviews <- read_csv("/Users/aoqu/Desktop/NoWait_Research/NMF/reviews(NMF).csv")
```
```{r}
...
if (nrow(df_regression_1)>0){
  model1 <- glm(complain_line ~ After + Treated + Treated_X_After + ratings + nfriends + useful + elite + nphotos_review + checkin + Fog_index + Avg_word_length + Polarity + Subjectivity + Sent_length + Quality_overall + Responsiveness + Food_quality + Atmosphere, data = df_regression_1, family = "binomial")
...
model2 <- glm(Wait_times ~ After + Treated + Treated_X_After + ratings + nfriends + useful + funny + cool + elite + nphotos_review + checkin + Fog_index + Avg_word_length + Polarity + Subjectivity + Sent_length, data = df_regression_2, family = "binomial")
...
if (nrow(df_regression_1)>0){
  model4 <- glm(complain_service ~ After + Treated + Treated_X_After + ratings + nfriends + useful + elite + nphotos_review + checkin + Fog_index + Avg_word_length + Polarity + Subjectivity + Sent_length + Quality_overall + Responsiveness + Food_quality + Atmosphere, data = df_regression_1, family = "binomial")
